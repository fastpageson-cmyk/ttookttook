...
- 2주차 이후 화면·Vercel 배포는 이번 범위 밖. **로컬 프리뷰만** (사용자 확정, 2026-07-18).

## 실행

```bash
npm install
npm run dev   # Vite, 기본 포트 5173
```

## 구조

- `src/App.jsx` — 화면 라우팅 (state 기반, react-router 없음)
- `src/state/store.jsx` — AppState reducer + localStorage 영속화 (`ddukdduk-state-v1`)
- `src/engine/` — 시뮬레이션 체결/평가, 리포트 계산, 규칙 기반 실수 리포트 (순수 함수, React 무관)
- `src/data/` — 목업 JSON 3종 + 진단 20문항·강의 10카드·퀴즈 5문항 (기획원본 md에서 코드로 옮긴 것 — 문구 수정 시 원본 md와 어긋나지 않게 주의)
