...
## 🎨 디자인 원칙 정비 (2026-07-22, main) — 이모지·중복·유틸리티

디터 람스 10원칙 기준 전면 점검 후 반영. **새 UI를 만들 때 아래를 지킬 것.**
- **OS 이모지 금지.** 사진풍 이모지는 플랫폼마다 다르게 렌더되고(원칙 7·9) 단색 시스템과 충돌한다.
  아이콘이 필요하면 `src/glyphs.js`의 단색 SVG 글리프(자물쇠·졸업모·차트·돋보기·책)를 쓰거나 새로 추가한다.
  글리프는 **`currentColor`만** 써서 부모 `color`(토큰)를 따른다. 퀴즈 정오답 `✓`/`✕`와 화살표 `→`는 타이포 기호라 유지.
- **의미 중복 요소 금지(원칙 10).** 같은 정보를 두 번 표시하지 말 것. 실제로 걷어낸 것들:
  홈 진도 행은 상태를 **왼쪽 타일 하나로만**(체크=완료/파란 점=진행/자물쇠=잠김), 뱃지는 '진행 중'만 남김
  (완료·잠김 뱃지 제거). 강의 진행표시는 진행바+카운터만(점 줄 `lc-dots` 제거). 실습 카운터는 하단 버튼 1곳만.
  졸업 '6주가 남긴 것'은 완료 뱃지 없음(완주해야 도달하므로 전제). 탭바는 아이콘 없이 텍스트만(2개뿐).
- **간격/정렬은 유틸리티 클래스**(`styles.css` 상단): `.mt-1~6`/`.mb-2~5`(8px 토큰), `.tac`/`.tar`, `.card.list`(행 나열 카드).
  새 코드는 인라인 `style`로 간격을 하드코딩하지 말고 이 유틸을 쓸 것. (기존 일회성 오프그리드 인라인 스타일은
  토큰과 안 맞아 그대로 둠 — 스냅하면 간격이 바뀌므로. 신규 코드만 유틸 준수.)
- **매수=빨강(`.btn.buy`)/매도=파랑(`.btn.sell`)** 전역 클래스. 조건부 `background` 인라인 대신 이 클래스.
- **종료 등 '다음 단계로' 액션은 무채색**(`.btn.ghost.tb-action`) — 위험색(빨강) 아님. `.btn{width:100%}`를 이기려면
  `.btn.tb-action` 특이성이 필요(단일 `.tb-action`은 진다 — 제목이 두 줄로 깨졌던 실제 버그).
