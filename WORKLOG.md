...
### 2026-07-18 세션 4 (토스 레퍼런스 디자인 개편)

- 레퍼런스: 루트의 `토스_레퍼런스.html`(토스팀 소개 페이지 저장본, git 미추적). 분석 결과를 반영해 **styles.css 토큰 전면 교체**:
  - 그레이 스케일 #191f28/#4e5968/#8b95a1/#e5e8eb/#f9fafb + 포인트 **토스블루 #3182f6** 단일. 다크 섹션 #121417.
  - **플랫 시스템**: 카드 그림자·보더 제거(`--shadow: none`), 흰 페이지 배경 + #f9fafb 틴트 블록으로 분리(기존: 회색 배경+흰 카드 → 반전).
  - 보조 버튼(ghost)은 토스식 **회색 채움**(아웃라인 없음). icon-btn·regime-chip은 필(pill). 본문 16px/행간 1.55, 제목 weight 800→700.
  - 시세·점수 숫자에 `font-variant-numeric: tabular-nums`, `:focus-visible` 포커스 링 추가.
  - JSX 하드코딩 색 일괄 치환(차트 #2b4acb→#3182f6, 다크 CTA #4a68e8→토스블루, 마커·기준금리선 등). 국내 관례(상승 빨강 #f04452/하락 파랑)는 유지.
- **스플래시 푸터 문제 해결**: 다크 히어로 아래 밝은 푸터가 튀던 것 → `.app:has(.splash) .app-footer`로 스플래시에서만 푸터를 같은 다크 배경+연한 글자(#6b7684)로 이어지게 함.
- 브라우저 검증(모바일 375/데스크톱 1280): 스플래시·진단결과·시뮬레이션 확인, 콘솔 에러 없음.
- 미적용 제안(사용자 결정 대기): 숫자 카운트업 모션, 강의 카드 아이콘/일러스트, 결과 공유 카드, 스켈레톤 로딩 등.
